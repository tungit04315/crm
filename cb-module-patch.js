...
window.cbUpdatePreview = function () {
    ...
    const html = `
    <div class="cb-watermark"><span>QUỐC TÙNG</span></div>
...
window.cbExportPDF = function () {
    ...
    const d = cbCollectData();
    // Tên tài liệu: TÊN KHÁCH HÀNG - MÃ HỢP ĐỒNG
    const customerName = (d.bName || '').trim() || 'Khách hàng';
    const contractNo = (d.contractNo || '').trim() || 'Draft';
    ...
    html2pdf().set({
        ...
        if (window.showToast) showToast('Đã xuất PDF thành công!', 'success');
        // Lưu lịch sử xuất vào Firestore (qua helper global của app.js)
        if (typeof window.saveExportHistory === 'function') {
            window.saveExportHistory(docName, htmlContent);
        }
    }).catch(err => {
        if (window.showToast) showToast('Lỗi xuất PDF: ' + (err.message || ''), 'error');
    });
};
...
window.cbExportDOCX = function () {
    ...
    // Lưu lịch sử xuất vào Firestore (qua helper global của app.js)
    if (typeof window.saveExportHistory === 'function') {
        window.saveExportHistory(docName, content);
    }
};
